...
if (!$email || !$password) {
    ...
    echo json_encode(["status" => "FAIL", "error" => "Email dan password harus diisi."]);
    exit;
}
...
try {
    ...
    if ($mahasiswa) {
        ...
        $_SESSION["npm"]  = $mahasiswa["NPM"];   // int
        echo json_encode([
            "status" => "OK",
            "role"   => "MAHASISWA",
            "id"     => $mahasiswa["NPM"],
    ...
    echo json_encode(["status" => "FAIL", "error" => "Email atau Password salah!"]);
